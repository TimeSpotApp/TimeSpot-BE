...
@Tag(
        ...
        description = """
                                      ...
                                      | API | 설명 |
                                      |-----|------|
                                      | `POST /api/v1/histories` | 여정 시작 (새로운 방문 이력 생성) |
                                      | `PUT /api/v1/histories/{historyId}` | 여정 종료 (완료 또는 포기) |
                                      | `GET /api/v1/histories` | 여정 목록 조회 (페이징, 검색 지원) |
                                      | `DELETE /api/v1/histories/{historyId}` | 여정 삭제 (통계 자동 조정) |
                      
                                      ### 🔔 알림 시스템 연동
                                      - **여정 시작 시**: 사용자의 알림 설정에 따라 푸시 알림이 자동 예약됩니다.
                                      - **알림 타입**: `DEPARTURE_TIME`, `BEFORE_5_MINUTES`, `BEFORE_10_MINUTES`, `BEFORE_15_MINUTES`, `END_JOURNEY`
                                      - **알림 기준 시간**: 열차 출발 시간 - (장소→역 도보 시간 + 플랫폼 대기 시간 10 분)
                                      - **여정 종료 시**: 예약된 모든 알림이 자동 취소됩니다.
                      
                                      ### 📊 여정 상태 흐름
                                      ```
                                      ┌─────────────┐
                                      │  여정 시작  │ → 알림 예약 (사용자 설정에 따라)
                                      │ (진행 중)   │
                                      └──────┬──────┘
                                             │
                              ┌──────────────┴──────────────┐
                      ...
                      ```
                                      ...
                                      ### ✅ 성공 판별 기준
                                      - `endTime ≤ trainDepartureTime`: **성공** (true) - 열차 출발 전 도착
                                      - `endTime > trainDepartureTime`: **실패** (false) - 시간 초과
                      
                                      ### 📈 통계 업데이트
                                      - **여정이 정상 완료된 경우에만** 사용자 통계와 즐겨찾기 방문 횟수가 업데이트됩니다.
                                      - 포기하거나 시간 초과한 경우 통계에 반영되지 않습니다.
                      """
)
