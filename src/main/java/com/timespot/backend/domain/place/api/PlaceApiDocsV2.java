    ...
    @Operation(
            ...
            description = """
                          ...
                          **`visitable` 은 다음 두 조건의 조합으로 결정됩니다:**
                          ...
                          1. **시간 기반 visitable** (기존 로직):
                             - 왕복 도보 시간 + 플랫폼 대기 시간 (10 분) + 최소 체류 시간 (10 분) 을 고려
                             - `stayableMinutes >= 10` → `visitable = true`
                          
                          2. **영업 상태** (Google Places API):
                             - `googleOpeningStatus = "영업 중"`
                          
                          **최종 `visitable` = 시간 기반 `visitable` && `영업 중`**
    ...
    @Operation(
            ...
            description = """
                          ...
                          - **시간 정보**:
                            ...
                            - `leaveTime`: 역으로 출발해야 하는 시간 (yyyy-MM-dd HH:mm:ss)
                              - **계산 방식**: `현재 시간 + (남은 시간 - 편도 도보 시간)`
                              - **의미**: 사용자가 이 장소에 있을 때, **최소 이 시간에는 역으로 출발**해야 도보로 역에 도착하여 열차를 탈 수 있음
                          ...
                          ```
                          - **의미**: 사용자가 현재 장소에 있을 때, **최소 이 시간에는 역으로 출발**해야 함
                          - **포함**: 도보 이동 시간 + 플랫폼 대기 시간 (10 분)
                          - **예시**:
                            - 현재 시간: 14:00
                            - 남은 시간: 60 분
                            - 편도 도보 시간: 10 분
