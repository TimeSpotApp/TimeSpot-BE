package com.nomadspot.backend.infra.redis.dao;
...
/**
 * PackageName : com.nomadspot.backend.infra.redis.dao
...
public class RedisRepositoryImpl implements RedisRepository {
    ...
    @Override
    public <T> Optional<T> rightPopFromList(final String key,
                                            final long timeout,
                                            final TimeUnit unit,
                                            final Class<T> type) {
    ...
    @Override
    public Set<Object> getZSetRangeByScore(final String key,
                                           final double min,
                                           final double max,
                                           final long offset,
                                           final long count) {
    ...
    @Override
    public Set<Object> getZSetReverseRangeByScore(final String key,
                                                  final double min,
                                                  final double max,
                                                  final long offset,
                                                  final long count) {
    ...
    @Override
    public TypedTuple<Object> popMinFromZSet(final String key) {
        return redisTemplate.opsForZSet().popMin(key);
    }

    @Override
    public Set<TypedTuple<Object>> popMinFromZSet(final String key, final long count) {
        Set<TypedTuple<Object>> set = redisTemplate.opsForZSet().popMin(key, count);
        if (set == null || set.isEmpty()) return Collections.emptySet();
        return set;
    }

    @Override
    public TypedTuple<Object> popMaxFromZSet(final String key) {
        return redisTemplate.opsForZSet().popMax(key);
    }

    @Override
    public Set<TypedTuple<Object>> popMaxFromZSet(final String key, final long count) {
        Set<TypedTuple<Object>> set = redisTemplate.opsForZSet().popMax(key, count);
        if (set == null || set.isEmpty()) return Collections.emptySet();
        return set;
    }
